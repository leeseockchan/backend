...
// 부모 클래스 (StarCraft)
// 공통 메소드 life()를 정의  . 자식 클래스들이 이를 오버라이드 한다.
// 자식 클래스 life() 메소드를 오버라이딩 하여 고유한 동작을 정의
// canon, run, fly 와 같은 각 클래스만의 고유 메소드를 추가.

// 메인 클래스
// 부모 클래스 배열 StarCraft[] 생성 후 다양한 자식 클래스를 저장
// 다형성을 활용하여 life() 메소드를 호출하여 자식 클래스의 오버라이드 된 메소드를 실행
// instance와 형변환을 사용하여 각 자식 클래스의 고유 메소드를 호출
